...
- **Submodule 展示**：包含 submodule 的仓库可在左侧展开；点击 submodule 后可在右侧查看详情和提交历史。
- **Submodule 初始化**：未初始化的 submodule 会显示状态和 URL，并可在右侧一键执行 `git submodule update --init`。
...
- 除右侧的 submodule 初始化按钮外，应用不会主动修改仓库。
